...
with tab2:
    ...
    st.markdown("""
        <div class="analysis-text">
            <strong>💡 Key Insight:</strong> The head-to-head comparison isolates historical matchups between specific franchises. Reviewing the direct win-loss ratios exposes psychological advantages and matchup dependencies that are obscured in aggregate seasonal standings. Outliers in these records often dictate future auction strategies.
        </div>
    """, unsafe_allow_html=True)
...
with tab3:
    ...
    st.markdown("""
        <div class="analysis-text">
            <strong>💡 Key Insight:</strong> Providing the raw dataset ensures transparency and allows users to trace the visual insights back to specific event occurrences. The table format is necessary for identifying edge cases, examining anomalies in venues, and auditing match outcomes at a granular level.
        </div>
    """, unsafe_allow_html=True)
...
with tab4:
    ...
    st.markdown("""
        <div class="analysis-text">
            <strong>💡 Key Insight:</strong> The line chart documents scheduling shifts and timeline disruptions, while the venue layout reveals spatial dependencies. The scatter plot provides crucial insight into win efficiency, plotting matches against wins. Teams positioned above the general cluster line represent positive outliers with superior conversion rates.
        </div>
    """, unsafe_allow_html=True)
